...
public class ResampleActivity extends Activity {
   ...
   public void loadVideoUri( Uri uri ) {
      ...
      String ol = " W/H" + mInputWidth +  mInputHeight + "BR:" +  MediaHelperAlone.GetBitRate( uri )+ "FR:" +  MediaHelperAlone.GetFrameRate( uri )
              + "FI:"+ MediaHelperAlone.GetIFrameInterval( uri ) ;

     Log.w("INPUT R-Video:", ol );
   }
   ...
   public void onResampleClicked( View view ) {
      ...
      String ol = " W/H" + mOutputResolution.getWidth() +   mOutputResolution.getHeight() + "BR:" +  mOutputBitRate+ "FR:" +  mOutputFrameRate
              + "FI:"+ mOutputIFrameInterval ;
      Log.d("OUTPUT R-Video:", ol );
      ResampleInBackgroundTask   rbn = new ResampleInBackgroundTask();
      rbn.mOutputBitRate = mOutputBitRate;  //2Mbps 2097152;
      rbn.mOutputFrameRate = mOutputFrameRate; // 30 fps
      rbn.mOutputIFrameInterval = mOutputIFrameInterval; // 1
      rbn.mOutputResolution = mOutputResolution; // 1080p



             rbn.execute( mInputUri , mOutputUri );
   }
