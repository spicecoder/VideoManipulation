...
import static com.littglobal.vidcomp.videomanipulation.MediaHelperAlone.MIME_TYPE_AVC;

@TargetApi( Build.VERSION_CODES.JELLY_BEAN_MR2 )
public class VideoResamplerAlone {
   ...
   InputSurfaceAlone mInputSurface;

   OutputSurfaceAlone  mOutputSurface;
   ...
   List<SamplerClipAlone> mClips = new ArrayList<SamplerClipAlone>();
   ...
   public void addSamplerClip( SamplerClipAlone clip ) {
      mClips.add( clip );
   }
   ...
   private void setupEncoder() {
      ...
      MediaFormat outputFormat = MediaFormat.createVideoFormat( MIME_TYPE_AVC, mWidth, mHeight );
      ...
      try {
         mEncoder = MediaCodec.createEncoderByType( MIME_TYPE_AVC );
      } catch (IOException e) {
         e.printStackTrace();
      }
      mEncoder.configure( outputFormat, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE );
      mInputSurface = new InputSurfaceAlone( mEncoder.createInputSurface() );
   ...
   private void resampleVideo() {
      ...
      for ( SamplerClipAlone clip : mClips ) {
   ...
   private void feedClipToEncoder( SamplerClipAlone clip ) {
      ...
      try {
         ...
         mOutputSurface = new OutputSurfaceAlone();
   ...
   private MediaExtractor setupExtractorForClip(SamplerClipAlone clip ) {
   ...
   private void resampleVideo( MediaExtractor extractor, MediaCodec decoder, SamplerClipAlone clip ) {
